...
class FactorGraph:
    def __init__(
        ...
        self._G = self._to_networkx(bayesian_network)
    ...
    @classmethod
    def _to_networkx(cls, bayesian_network: BayesianNetwork):
        ...
        for node in bayesian_network.nodes:
            variable_node = f"V_{node.name}"
            variable_nodes[node] = variable_node
            G.add_node(variable_node, node=node)

            factor_node = f"F_{node.name}"
            factor_nodes[node] = factor_node
            G.add_node(factor_node, node=node)
